...
s.on('message', function(msg, info) {
	...
	match = re.exec(text);
	if(match != null) {
		for (var prop in servers[addr].state.playerrs) { if (servers[addr].state.players.hasOwnProperty(prop)) { delete servers[addr].state.players[prop]; } }
	}
	...
	match = re.exec(text);
	if(match != null) {
		var score = { 'TERRORIST': parseInt(match.capture('t_score')), 'CT': parseInt(match.capture('ct_score')) };
...
function Server (address, pass, adminip) {
	...
	this.clantag = function(team) {
		...
		var tags = {};
		var ret = 'noobs';
		if(team == 'CT' && this.clantag('TERRORIST') == 'noobs') {
			ret = 'lamers';
		}
	...
	this.stats = function (tochat) {
		...
		for (var i = 0; i < this.state.maps.length; i++) {
			maps.push(this.state.maps[i]+' '+stat[team1][i]+'-'+stat[team2][i]);
			scores.push(stat[team1][i]+'-'+stat[team2][i]);
		}
		...
		if (this.state.live && this.state.paused) {
			...
			if (this.state.ready['TERRORIST'] != this.state.ready['CT']) {
				...
				if(this.state.knife) {
					...
					setTimeout(function(){tag.rcon(KNIFE_STARTED);}, 9000);
				} else {
					this.rcon(MATCH_STARTING);
					setTimeout(function(){tag.rcon(MATCH_STARTED);}, 9000);
				}
				setTimeout(function(){tag.rcon('say \x054...');}, 1000);
				setTimeout(function(){tag.rcon('say \x063...');}, 2000);
				setTimeout(function(){tag.rcon('say \x102...');}, 3000);
				setTimeout(function(){tag.rcon('say \x0f1...');}, 4000);
				setTimeout(function(){tag.rcon(LIVE);}, 5000);
			}
		}
	}
...
setInterval(function () {
	for(var i in servers) {
		...
		} else if(servers[i].state.freeze) {
